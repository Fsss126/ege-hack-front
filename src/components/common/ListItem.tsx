...
export type ListItemClickHandler<T = any, P = {}> = (item: T, callbackProps: P) => void;
...
} & ({truncate: true; description?: string} | {truncate: false; description?: React.ReactNode})
const ListItem = <T extends {} = any, P = undefined>(props: ListItemProps<T, P>): React.ReactElement => {
    const {
    ...
    const [descriptionRef, isFontLoaded] = useTruncate(typeof description === 'string' ? description : undefined);
    const onClick = useCallback<React.MouseEventHandler<HTMLElement>>((event) => {
    ...
    const onActionClick = useCallback<React.MouseEventHandler<HTMLDivElement>>(() => {
