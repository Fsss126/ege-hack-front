...
export const isWebinarUnlocked = (param: Webinar | CountdownTimeDelta): boolean => {
    if ('total' in param) {
        ...
        const webinar = param;
        const now = new Date();
        return now >= new Date(webinar.date_start.getTime() - UNLOCK_TIME) && now < webinar.date_end;
    }
};
...
const Webinar: React.FC<WebinarProps> = ({webinar, courseId}) => {
    ...
    const onClick = React.useCallback(async (event) => {
        ...
        const response = await APIRequest.get(`/courses/${webinar.course_id || courseId}/schedule/link`) as LinkResp;
...
const WebinarSchedule: React.FC<WebinarScheduleProps> = ({schedule, courseId}) => {
