import React, {Fragment, Ref, RefAttributes, useCallback, useRef, useState} from "react";
...
export type FormSubmitHandler<A extends Array<any>, R extends Promise<any> | void = void> = (...args: A) => R;
export type FormStateHookResult<A extends Array<any>, R, E extends Error = Error> = {
    submitting: boolean | null;
    hasChanged: boolean;
    onChange: SimpleCallback;
    handleSubmit: (...params: A) => Promise<R> | void;
}
export function useFormState<A extends Array<any>, R, E extends Error = Error>(callback: FormSubmitHandler<A, Promise<R> | void>, onSubmitted?: (result: R) => void, onError?: (error: E, retry: SimpleCallback) => void): FormStateHookResult<A, R, E> {
    ...
    const handleSubmit = React.useCallback((...params: A) => {
...
export function useForm<D extends FormData>(initFormData: (data?: D) => D, checkValidity?: FormValidityChecker<D>): FormHookResult<D> {
...
export const FormElement: React.withDefaultProps<React.FC<FormElementProps>> = (props) => {
...
export const FormElementGroup = <E extends {}>(props: FormElementGroupProps<E>): React.ReactElement => {
    const {name, onChange, elements, renderElement, maxElements, initialElementData, addBtnText} = props;
...
export type ShowSuccessMessageCallback = (message: string, actions: MessageAction[], modal?: boolean) => void;
export type ShowErrorMessageCallback = (message: string, actions: MessageAction[]) => void;

export type FormSubmittedHandler<R> = (response: R, showSuccessMessage: ShowSuccessMessageCallback, reset: SimpleCallback) => void;
export type FormErrorHandler<E extends Error = AxiosError> = (error: E, showErrorMessage: ShowErrorMessageCallback, reloadCallback: SimpleCallback) => void;
...
const Form = <R, E extends Error = AxiosError>(props: FormProps<R, E>, ref: React.Ref<HTMLFormElement>): React.ReactElement => {
    const {
        title,
        id,
        className,
    ...
    const handleSubmitted = React.useCallback((response: R) => {
        const messagePopup = messagePopupRef.current;
        const showSuccessMessage: ShowSuccessMessageCallback = (message, actions, modal = true) => {
            if (messagePopup) {
    ...
    const handleError = React.useCallback((error: E, reloadCallback: SimpleCallback) => {
        const messagePopup = messagePopupRef.current;
    ...
    const {submitting, handleSubmit, hasChanged, onChange} = useFormState<[undefined], R, E>(onSubmit, handleSubmitted, handleError);
...
const FormWithRef = React.forwardRef(Form) as any as <R, E extends Error = AxiosError>(props: FormProps<R, E> & RefAttributes<HTMLFormElement>) => React.ReactElement;
