...
import Dropzone, {IFileWithMeta, IMeta, IDropzoneProps, IInputProps} from "react-dropzone-uploader";
...
export const FileInputContext = React.createContext<FileInputContextState>(undefined as any);
...
export const allFilesReady = (files: IFileWithMeta[]): boolean => files.every(f => isFileReady(f));

export function useInputCallback(getFilesFromEvent: IInputProps['getFilesFromEvent'], onFiles: IInputProps['onFiles']): React.ChangeEventHandler<HTMLInputElement> {
...
export function usePreviewState(fileWithMeta: IFileWithMeta, meta: IMeta, isUpload: boolean): PreviewState {
...
const getCallbackFiles = (files: IFileWithMeta[], preloadedFiles: FileInfo[]): FileInfo[] | undefined => {
...
const GenericFileInput = (props: FileInputProps): React.ReactElement => {
    ...
    const handleChangeStatus: Required<IDropzoneProps>['onChangeStatus'] = useCallback((file, status, files) => {
        onChange();
