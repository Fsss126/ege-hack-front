...
import Form, {
    FieldsContainer,
    FormElement,
    FormElementGroup, FormErrorHandler,
    FormProps, FormSubmitHandler,
...
const WebinarsForm: React.FC<WebinarsFormProps> = (props) => {
    ...
    const checkValidity = useFormValidityChecker<WebinarsFormData>(formElementRef.current, (name, input, formData) => {
        if (name === 'image') {
    ...
    const {
        ...
        onInputChange,
        reset
    } = useForm<WebinarsFormData>((state): WebinarsFormData => {
    ...
    const onSubmit = useCallback<FormSubmitHandler<[undefined], Promise<WebinarScheduleInfo>>>(() => {
        console.log('submit', formData);
        return createRequest(getRequestData(formData));
    }, [formData, createRequest]);

    const onError = useCallback<FormErrorHandler>((error, showErrorMessage, reloadCallback) => {
    ...
    return (
        <Form<WebinarScheduleInfo>
            ...
            <FormElementGroup<WebinarData>
                ...
                renderElement={({name, date_start, duration, description}, i): React.ReactElement => (
                    <FormElement
                        name="webinars"
                        index={i}
                        onChange={onInputChange}
