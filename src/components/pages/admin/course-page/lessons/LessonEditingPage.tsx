...
const LessonEditingPage: React.FC<RouteComponentProps<{courseId: string; lessonId: string}>> = (props) => {
    const {match: {params: {courseId: param_course, lessonId: param_lesson}}, location} = props;
    const courseId = parseInt(param_course);
    const lessonId = parseInt(param_lesson);

    const createRequest = useCallback((requestData: LessonDtoReq): Promise<LessonInfo> => APIRequest.put(`/lessons/${lessonId}`, requestData), [lessonId]);

    const {lesson, error: errorLoadingLesson, reload: reloadLesson} = useLesson(courseId, lessonId);
