...
import Form, {
    FieldsContainer, FormErrorHandler,
...
const CourseForm: React.FC<CourseFormProps> = (props) => {
    ...
    const teacherOptions = useMemo<TeacherOption[]>(() =>
        teachers.map(({id,  vk_info: {first_name, last_name}}) => ({value: id, label: `${first_name} ${last_name}`})),
        [teachers]);

    const formElementRef = useRef<HTMLFormElement>(null);

    const checkValidity = useFormValidityChecker<CourseFormData>(formElementRef.current, (name, input, formData) => {
        if (name === 'image') {
    ...
    const {
        ...
        onInputChange,
        reset
    } = useForm<CourseFormData>((state): CourseFormData => {
    ...
    const onSubmit = useCallback<FormSubmitHandler<[undefined], Promise<CourseInfo>>>(() => {
